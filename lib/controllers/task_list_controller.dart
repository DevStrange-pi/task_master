    ...
    for (var task in tasksList) {
      if (task.status == "pending" && task.deadline != null) {
        ...
        if (task.type!.toLowerCase() == "daily") {
          ...
          final todayEnd = DateTime(nowLocal.year, nowLocal.month, nowLocal.day, 23, 59, 59);
          diffSeconds = todayEnd.millisecondsSinceEpoch ~/ 1000 - nowLocal.millisecondsSinceEpoch ~/ 1000;
        } else {
          final deadline = task.deadline!;
          String formattedDeadline = formatDateTime(deadline);
          final newDeadline = DateTime.parse(formattedDeadline);
          diffSeconds = newDeadline.millisecondsSinceEpoch ~/ 1000 - now.millisecondsSinceEpoch ~/ 1000;
        }
        if (diffSeconds < 0) {
          diffSeconds = 0;
          updated.add("");
